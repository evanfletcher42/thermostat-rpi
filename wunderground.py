...
def update_weather():
    ...
    if time.time() - __lastUpdateTime > __MIN_UPDATE_PERIOD_SECONDS:
        try:
            ...
            f = urllib2.urlopen('http://api.wunderground.com/api/WUNDERGROUND_API_KEY/geolookup/conditions/q/pws:KCOBOULD86.json', timeout=1)
            json_string = f.read()
...
def getTempC():
    ...
    if __last_parsed_json:
        return (datetime.fromtimestamp(rfc822.mktime_tz(rfc822.parsedate_tz(__last_parsed_json['current_observation']['observation_time_rfc822']))), \
