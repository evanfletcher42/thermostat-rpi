...
static volatile boolean timer_pwm_disabled = false;
static volatile int timer_pwm_count = 0;
#endif
...
// Note: first bit must be a one (start bit)
void IRsend::sendGEAC(unsigned char *data, int nBits)
{
  enableIROut(37);
...
#ifdef __RFduino__
  timer_pwm_count = time / NRF_TIMER1->CC[0];
  while(timer_pwm_count > 0);
#else
...
#ifdef __RFduino__
void timer_config_disabled() {
  // Turn off
  return;
  NRF_TIMER1->TASKS_STOP = 1;   // Stop timer
  digitalWrite(TIMER_PWM_PIN, LOW); // be sure led is low
}

ISR(TIMER_INTRS_NAME) {
  if ((NRF_TIMER1->EVENTS_COMPARE[1] != 0) && ((NRF_TIMER1->INTENSET & TIMER_INTENSET_COMPARE1_Msk) != 0)) {
    NRF_TIMER1->EVENTS_COMPARE[1] = 0;
    NRF_GPIO->OUTCLR = (1 << TIMER_PWM_PIN);
    if (timer_pwm_count > 0) timer_pwm_count--;
    // digitalWrite(TIMER_PWM_PIN, LOW);
  }
  if ((NRF_TIMER1->EVENTS_COMPARE[0] != 0) && ((NRF_TIMER1->INTENSET & TIMER_INTENSET_COMPARE0_Msk) != 0)) {
    NRF_TIMER1->EVENTS_COMPARE[0] = 0;
    if (TIMER_PWM_ISDISABLED) return;
      NRF_GPIO->OUTSET = (1 << TIMER_PWM_PIN);
   // digitalWrite(TIMER_PWM_PIN, HIGH);
  }
}

void timer_config_khz(int khz) {
  void TIMER_INTRS_NAME(void);
  // Configure Timer 1
  float val = 1000.0 / khz;
  NRF_TIMER1->TASKS_STOP = 1;   // Stop timer
  NRF_TIMER1->MODE = TIMER_MODE_MODE_Timer;  // taken from Nordic dev zone
  NRF_TIMER1->BITMODE = TIMER_BITMODE_BITMODE_16Bit;//0x01UL;
  NRF_TIMER1->PRESCALER = 4;   // Max 9, SysClk/2^PRESCALER) =  16,000,000/16 = 1us resolution
  NRF_TIMER1->TASKS_CLEAR = 1;  // clear the task first to be usable for later
  NRF_TIMER1->CC[0] = (int)(val + 0.5);
  NRF_TIMER1->CC[1] = (int)(val / 3.0 + 0.5);
  NRF_TIMER1->EVENTS_COMPARE[0] = 0;
  NRF_TIMER1->EVENTS_COMPARE[1] = 0;
  // Interrupt setup
  ...
  attachInterrupt(TIMER1_IRQn, TIMER_INTRS_NAME);
  // Start clock
...
void timer_config_normal(void) {
  ...
  NRF_TIMER1->CC[0] = 50; // Cannot exceed 16bits
  NRF_TIMER1->EVENTS_COMPARE[0] = 0;
  // Interrupt setup
  NRF_TIMER1->INTENCLR = 0xFFFFFFFF;
  NRF_TIMER1->INTENSET = TIMER_INTENSET_COMPARE0_Msk;  // taken from Nordic dev zone
  // Enable Shortcut between CC[0] event and the CLEAR task
  NRF_TIMER1->SHORTS = TIMER_SHORTS_COMPARE0_CLEAR_Msk;
  attachInterrupt(TIMER1_IRQn, TIMER_INTR_NAME);    // also used in variant.cpp to configure the RTC1
  // Start clock
  NRF_TIMER1->TASKS_START = 1;   // Start TIMER
}
#endif
